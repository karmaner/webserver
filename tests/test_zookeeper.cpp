...
void on_watcher(int type, int stat, const std::string& path, webserver::ZKClient::ptr client) {
    WEBSERVER_LOG_INFO(g_logger) << " type=" << type
        << " stat=" << stat
        << " path=" << path
        << " client=" << client
    ...
    if(stat == ZOO_CONNECTED_STATE) {
        if(g_argc == 1) {
            ...
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "[" << webserver::Join(vals.begin(), vals.end(), ",") << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "getChildren error " << rt;
            }
        } else {
            std::string new_val;
            new_val.resize(255);
            ...
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "[" << new_val.c_str() << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "getChildren error " << rt;
            }
...
//extern ZOOAPI const int ZOO_CONTAINER;
            ...
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "create [" << new_val.c_str() << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "create error " << rt;
            }

            rt = client->get("/hello", new_val, true);
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "get [" << new_val.c_str() << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "get error " << rt;
            }
            ...
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "set [" << new_val.c_str() << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "set error " << rt;
            }

            rt = client->del("/hello");
            if(rt == ZOK) {
                WEBSERVER_LOG_INFO(g_logger) << "del [" << new_val.c_str() << "]";
            } else {
                WEBSERVER_LOG_INFO(g_logger) << "del error " << rt;
            }
...
int main(int argc, char** argv) {
    ...
    if(g_argc > 1) {
        WEBSERVER_LOG_INFO(g_logger) << client->init("127.0.0.1:21811", 3000, on_watcher);
        //WEBSERVER_LOG_INFO(g_logger) << client->init("127.0.0.1:21811,127.0.0.1:21812,127.0.0.1:21811", 3000, on_watcher);
