...
int32_t StatusServlet::handle(webserver::http::HttpRequest::ptr request
...
#define XX(key) \
    ...
    std::stringstream ss;
    ss << "===================================================" << std::endl;
    XX("server_version") << "webserver/1.0.0" << std::endl;
    ...
    ss << "===================================================" << std::endl;
    XX("fibers") << webserver::Fiber::TotalFibers() << std::endl;
    ss << "===================================================" << std::endl;
    ss << "<Logger>" << std::endl;
    ss << webserver::LoggerMgr::GetInstance()->toYamlString() << std::endl;
    ss << "===================================================" << std::endl;
    ss << "<Woker>" << std::endl;
    webserver::WorkerMgr::GetInstance()->dump(ss) << std::endl;

    std::map<std::string, std::vector<TcpServer::ptr> > servers;
    webserver::Application::GetInstance()->listAllServer(servers);
    ss << "===================================================" << std::endl;
    for(auto it = servers.begin();
        ...
        webserver::http::HttpServer::ptr hs;
        for(auto iit = it->second.begin();
            ...
            if(!hs) {
                hs = std::dynamic_pointer_cast<webserver::http::HttpServer>(*iit);
            }
