...
void Table::gen_inc(const std::string& path) {
    ...
    std::set<std::string> incs = {"webserver/db/db.h", "webserver/util.h"};
    for(auto& i : incs) {
        ofs << "#include \"" << i << "\"" << std::endl;
    }
    ofs << std::endl;
...
void Table::gen_src(const std::string& path) {
    ...
    ofs << "#include \"webserver/log.h\"" << std::endl;
    ofs << std::endl;
    ...
    ofs << std::endl;
    ofs << "static webserver::Logger::ptr g_logger = WEBSERVER_LOG_NAME(\"orm\");" << std::endl;
...
#define CHECK_STMT(v) \
    ...
    ofs << "        WEBSERVER_LOG_ERROR(g_logger) << \"stmt=\" << sql" << std::endl << \
        "                 << \" errno=\"" \
