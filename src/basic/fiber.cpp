...
class MallocStackAllocator { 	// 协程栈分配器
public:
	static void* Alloc(size_t size) {
		return malloc(size);
	}

	static void Dealloc(void* vp, size_t size) { 	// 释放协程栈
		return free(vp);
	}
};

using StackAllocator = MallocStackAllocator;	// 使用malloc分配协程栈

uint64_t Fiber::GetFiberId() {	// 获取当前协程id
	if(t_fiber) {
