...
EMailEntity::ptr EMailEntity::CreateAttach(const std::string& filename) {
    ...
    entity->m_content = webserver::base64encode(entity->m_content);
    entity->addHeader("Content-Transfer-Encoding", "base64");
    entity->addHeader("Content-Disposition", "attachment");
    entity->addHeader("Content-Type", "application/octet-stream;name=" + webserver::FSUtil::Basename(filename));
    return entity;
}
