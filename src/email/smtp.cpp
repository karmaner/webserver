...
SmtpClient::ptr SmtpClient::Create(const std::string& host, uint32_t port, bool ssl) {
    ...
    Socket::ptr sock;
    if(ssl) {
        sock = webserver::SSLSocket::CreateTCP(addr);
    } else {
    ...
    buf.resize(len);
    if(webserver::TypeUtil::Atoi(buf) != 220) {
        return nullptr;
    }
    rt->m_host = host;
    return rt;
...
SmtpResult::ptr SmtpClient::doCmd(const std::string& cmd, bool debug) {
    ...
    int code = webserver::TypeUtil::Atoi(buf);
    if(code >= 400) {
...
#define DO_CMD() \
    ...
    if(!m_authed && !email->getFromEMailAddress().empty()) {
        ...
        cmd = webserver::base64encode(email->getFromEMailAddress().substr(0, pos)) + "\r\n";
        DO_CMD();
        cmd = webserver::base64encode(email->getFromEMailPasswd()) + "\r\n";
        DO_CMD();
...
#define XX(fun) \
    ...
    std::string boundary;
    if(!entitys.empty()) {
        boundary = webserver::random_string(16);
