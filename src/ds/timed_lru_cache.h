#ifndef __SRC_DS_TIMED_LRU_CACHE_H__
#define __SRC_DS_TIMED_LRU_CACHE_H__
...
template<class K, class V, class MutexType = webserver::Mutex>
class TimedLruCache {
private:
    struct Item {
...
public:
    ...
    void set(const K& k, const V& v, uint64_t expired) {
        ...
        if(it != m_cache.end()) {
            ...
            it->second->val = v;
            it->second->ts = expired + webserver::GetCurrentMS();
            m_timed.insert(it->second);
            return;
        }

        m_keys.emplace_front(Item(k, v, expired + webserver::GetCurrentMS()));
    ...
    size_t checkTimeout(const uint64_t& ts = webserver::GetCurrentMS()) {
        size_t size = 0;
        typename MutexType::Lock lock(m_mutex);
...
template<class K, class V, class MutexType = webserver::Mutex, class Hash = std::hash<K> >
