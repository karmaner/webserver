...
void FoxThread::read_cb(evutil_socket_t sock, short which, void* args) {
    ...
    if(recv(sock, cmd, sizeof(cmd), 0) > 0) {
        ...
        webserver::Atomic::addFetch(thread->m_total, callbacks.size());
...
bool FoxThreadPool::dispatch(callback cb) {
    do {
        webserver::Atomic::addFetch(m_total, (uint64_t)1);
        RWMutex::WriteLock lock(m_mutex);
        if(!m_advance) {
...
bool FoxThreadPool::batchDispatch(const std::vector<callback>& cbs) {
    webserver::Atomic::addFetch(m_total, cbs.size());
    RWMutex::WriteLock lock(m_mutex);
    if(!m_advance) {
        for(auto cb : cbs) {
...
const std::string& FoxThread::GetFoxThreadName() {
    ...
    uint64_t tid = webserver::GetThreadId();
    do {
...
void FoxThread::setThis() {
    m_name = m_name + "_" + std::to_string(webserver::GetThreadId());
    s_thread = this;

    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names[webserver::GetThreadId()] = m_name;
}

void FoxThread::unsetThis() {
    s_thread = nullptr;
    RWMutex::WriteLock lock(s_thread_mutex);
    s_thread_names.erase(webserver::GetThreadId());
}
...
void FoxThreadManager::init() {
    ...
    for(auto i : m) {
        auto num = webserver::GetParamValue(i.second, "num", 0);
        auto name = i.first;
