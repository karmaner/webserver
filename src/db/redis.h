#ifndef __SRC_DB_REDIS_H__
#define __SRC_DB_REDIS_H__
...
public:
    ...
    FoxRedis(webserver::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedis();
...
public:
    ...
    FoxRedisCluster(webserver::FoxThread* thr, const std::map<std::string, std::string>& conf);
    ~FoxRedisCluster();
...
typedef webserver::Singleton<RedisManager> RedisMgr;
